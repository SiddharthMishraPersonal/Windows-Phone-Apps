...
{
    public class ProgramsDataSource : DataSourceBase<ProgramMaster>
    {
        ...
        public async override Task<IEnumerable<ProgramMaster>> LoadDataAsync()
        {
            try
            {
                var serviceDataProvider = new ServiceDataProvider(_appId, _dataSourceName);
                return await serviceDataProvider.Load<ProgramMaster>();
            }
            catch (Exception ex)
            {
                AppLogs.WriteError("ProgramsDataSource.LoadData", ex.ToString());
                return new ProgramMaster[0];
            }
        }
    }
}
